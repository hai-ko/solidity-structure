...
module.exports = {
  ...
  parse: function (annotationString, _options = {}) {
    ...
    annotationString.split(/\r?\n/).forEach(str => {
      ...
      for (let titleParam of ['@title']) {
        if (str.startsWith(titleParam + ' ')) {
          annotation.title = str.substring(titleParam.length + 1);
        }
      }

      for (let titleParam of ['@dev']) {
        if (str.startsWith(titleParam + ' ')) {
          annotation.dev = str.substring(titleParam.length + 1);
        }
      }

      for (let titleParam of ['@notice']) {
        if (str.startsWith(titleParam + ' ')) {
          annotation.notice = str.substring(titleParam.length + 1);
        }
      }
